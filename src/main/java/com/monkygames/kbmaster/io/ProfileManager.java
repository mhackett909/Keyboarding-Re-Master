...
// ============= Constructors ============== //
	...
	public boolean importProfile(File file) {
		...
		try {
			...
			if (!deviceName.equals(profileUIController.getDevice().getDeviceInformation().getName())) return false;
			App app = profileUIController.getAppByName(appType.toString(), appName);
			if (app == null) {
				addApp(new App("", null, null, appName, deviceName, appType));
